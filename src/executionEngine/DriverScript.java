...
public class DriverScript {
  ...
  //Second method, this is to figure out the test cases execution one by one
      ...
      private void execute_TestCase() throws Exception {
  	    	...
  	    	//Clear Excel file fields 	    	
  	    	for(int iTestcase=1;iTestcase<iTotalTestCases;iTestcase++){
  	    	ExcelUtils.setCellData(Constants.KEYWORD_NONE,iTestcase,Constants.Col_Result,Constants.Sheet_TestCases);	
			...
			iTestLastStep = ExcelUtils.getTestStepsCount(Constants.Sheet_TestSteps, sTestCaseID, iTestStep);
  	    	...
  	    	for (;iTestStep<iTestLastStep;iTestStep++){		
  	    		ExcelUtils.setCellData(Constants.KEYWORD_NONE, iTestStep, Constants.Col_TestStepResult, Constants.Sheet_TestSteps);	    		
  	    	}
       ...
       private static void execute_Actions() throws Exception {
  		...
  		for(int i=0;i<method.length;i++){
  			...
  			if(method[i].getName().equals(sActionKeyword)){
  				...
  				//Fill the excel with pass\fail for each step --> This code block will execute after every test step -
  				if(bResult==true){
